<?php
    ...
    if(!defined("_UTILS")) {
	 ...
	 class Mydb {
		...
		/**
		 * Get and set default Mydb configurations
		 * @uses   static::config
		 ...
		 * @return Mydb Singleton instance
		 */
		...
		public function __construct ( $driver, $host, $database, $user, $password = null ) {
			...
			$this->mydb = new pdo( $driver . ':host=' . $host . ';dbname=' . $database, $user, $password, array(
		...
		public function query ( $sql, array $params ) {
			$this->result = isset( $this->statement[ $sql ] ) ?
				$this->statement[ $sql ] :
				$this->statement[ $sql ] = $this->mydb->prepare( self::_uncomment( $sql ) );
